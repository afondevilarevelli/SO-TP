...
rtdoEjec_t accederRecurso(op_t operacion, char * clave, char * valor)
{
	...
	case SET: rtdo = setRecurso(clave, valor);
		break;

	case STORE: rtdo = storeRecurso(clave); break;
	case ORDEN_COMPACTAR: compactar(); break;
		default:
			log_error(pLog, "Se intento realizar una operacion invalida");
			break;
	}
	if (operacion!=COMPACTAR)actualizarRegistro(clave);
	return rtdo;
}
...
rtdoEjec_t agregarEntrada(char * clave, char *valor)
{
	...
	static int pointer=0;
			...
			if(entryCant*entrySize<=(pointer+cantEntradaNecesarias)){//Se termino la cantidad de lugar. el tamaño del almacenaiento debe volver atras
				if(yaCompacto){
...
char * obtenerValor(char * clave){
	...
	for(int i =0;i!=pEntry->tamanioValor;i++){
